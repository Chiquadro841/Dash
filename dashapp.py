...
# Carica i dati dal file CSV per il modello SARIMAX
csv_data = pd.read_csv('sp500_monthly_data.csv')
...
# Scarica i dati da Yahoo Finance per il grafico
ticker = 'ES=F'  # Simbolo per l'S&P 500 ETF, puoi cambiarlo con un altro simbolo se preferisci
yfinance_data = yf.download(ticker, start="2000-01-01", end=pd.to_datetime("today").strftime('%Y-%m-%d'), interval='1mo')

# Filtra solo la colonna 'Close' per il grafico
...
def update_forecast(n_clicks, y_scale, periods, eps, fedfunds, stored_data, xaxis_range, relayout_data):
    ...
    if trigger == 'submit' and n_clicks > 0:
        ...
        future_dates = pd.date_range(start=yfinance_data.index[-1] + pd.DateOffset(months=1), periods=periods, freq='M')
        ...
        # Modello SARIMAX basato sui dati del CSV
        order = (1, 1, 1)
        seasonal_order = (1, 1, 1, 12)

        model = SARIMAX(csv_data['Close'], exog=csv_data[['Eps', 'FedFundsRate']], order=order, seasonal_order=seasonal_order)
        results = model.fit()
    ...
    # Aggiungi i dati storici da yfinance per il grafico
    fig.add_trace(go.Scatter(x=yfinance_data.index, y=yfinance_data['Close'], mode='lines+markers', name='Storico'))
    ...
    y_min = yfinance_data['Close'].min() * y_scale
    y_max = yfinance_data['Close'].max() * y_scale
